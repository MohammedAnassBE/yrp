...
def _stock_context():
	...
	uom = frappe.db.get_value("Item", item, "default_unit_of_measure")
	if not uom:
...
class TestStockValuationAdjustment(FrappeTestCase):
	...
	def test_adjustment_revalues_remaining_stock_without_changing_original_rate(self):
		...
		self.assertEqual(frappe.db.get_value("Stock Ledger Entry", sle_name, "valuation_is_stale"), 0)
		...
		with self.assertRaisesRegex(frappe.ValidationError, "immutable audit"):
			frappe.get_doc(
				"Stock Valuation Adjustment", adjustment
			).before_cancel()
		...
		self.assertAlmostEqual(
			flt(
				frappe.db.get_value(
					"Stock Ledger Entry", sle_name, "valuation_adjustment_value"
				)
			),
			10,
		)
	...
	def test_reversal_waits_until_the_original_adjustment_is_applied(self):
		...
		with patch(
			...
			self.assertEqual(
				frappe.db.get_value(
					"Stock Valuation Adjustment", original, "status"
				),
				"Reversal Queued",
			)
		...
		self.assertEqual(
			frappe.db.get_value("Stock Valuation Adjustment", original, "status"),
			"Reversed",
		)
		self.assertEqual(
			frappe.db.get_value("Stock Ledger Entry", sle_name, "valuation_is_stale"),
			0,
		)
	...
	def test_voucher_cannot_cancel_while_an_adjustment_owns_its_receipt(self):
		...
		self.assertEqual(
			frappe.db.get_value(
				"Stock Valuation Adjustment", adjustment, "status"
			),
			"Queued",
		)
	...
	def test_closed_period_blocks_adjustment_and_reversal_before_enqueue(self):
		...
		self.assertFalse(
			frappe.db.exists(
				"Stock Valuation Adjustment", {"reversal_of": original, "docstatus": 1}
			)
		)
		self.assertEqual(
			frappe.db.get_value("Stock Valuation Adjustment", original, "status"),
	...
	def test_effective_date_is_the_earliest_affected_receipt_date(self):
		...
		self.assertEqual(
			getdate(
				frappe.db.get_value(
					"Stock Valuation Adjustment", adjustment, "effective_date"
				)
			),
	...
	def test_adjustment_follows_persisted_transfer_pair(self):
		...
		incoming_overlay = frappe.db.get_value(
			"Stock Ledger Entry", incoming.name, "valuation_adjustment_value"
		)
		parent = frappe.db.get_value(
			"Stock Valuation Adjustment",
			adjustment,
	...
	def test_same_rate_fifo_receipts_keep_late_cost_on_the_consumed_layer(self):
		...
		self.assertAlmostEqual(
			flt(
				frappe.db.get_value(
					"Stock Ledger Entry", incoming, "valuation_adjustment_value"
				)
			),
			5,
		)
		...
		self.assertEqual(
			frappe.db.get_value("Stock Valuation Adjustment", adjustment, "status"),
			"Reversed",
		)
	...
	def test_adjustment_crosses_a_persisted_production_link(self):
		...
		self.assertEqual(
			frappe.db.get_value("Stock Ledger Entry", output_sle, "is_cancelled"), 0
		)
		self.assertEqual(
		...
		self.assertEqual(
			frappe.db.get_value("Stock Ledger Entry", output_sle, "is_cancelled"), 1
		)
		self.assertEqual(
			frappe.db.get_value("Stock Valuation Production Link", link_name, "active"),
			0,
