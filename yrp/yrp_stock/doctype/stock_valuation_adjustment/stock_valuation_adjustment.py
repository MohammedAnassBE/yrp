...
class StockValuationAdjustment(Document):
	def validate(self):
...
def create_adjustment(
	*,
	...
	existing = frappe.db.get_value(
		"Stock Valuation Adjustment", {"idempotency_key": idempotency_key}, "name"
	)
	if existing:
		return existing
	...
	doc = frappe.get_doc(
		{
			"doctype": "Stock Valuation Adjustment",
	...
	try:
		...
		existing = frappe.db.sql(
			"""
			SELECT name
			FROM `tabStock Valuation Adjustment`
...
def _lock_adjustment_creation():
	...
	frappe.db.sql(
		...
		SELECT name
		FROM `tabDocType`
		WHERE name='Stock Valuation Adjustment'
		FOR UPDATE
		"""
	)
...
def _validate_no_conflicting_adjustments(
	...
	rows = frappe.db.sql(
		...
		FROM `tabStock Valuation Adjustment Source` source
		INNER JOIN `tabStock Valuation Adjustment` adjustment
...
def create_purchase_invoice_adjustment(invoice):
	...
	for grn_name in grn_names:
		...
		work_order = None
		if grn.against == "Work Order":
			work_order = work_orders.setdefault(
				grn.against_id, frappe.get_doc("Work Order", grn.against_id)
			)
...
def create_reversal(source_doctype, source_name, source_event="cancel"):
	...
	for adjustment in adjustments:
		rows = frappe.get_all(
			...
			filters={"parent": adjustment.name, "parenttype": "Stock Valuation Adjustment"},
			fields=[
				"source_row",
				"source_grn",
				"source_grn_item",
...
def _lock_adjustment(adjustment):
	rows = frappe.db.sql(
		"SELECT * FROM `tabStock Valuation Adjustment` WHERE name=%s FOR UPDATE",
		(adjustment,),
		as_dict=True,
	)
	if not rows:
...
def _build_initial_entries(doc):
	rows = frappe.get_all(
		"Stock Valuation Adjustment Source",
		filters={"parent": doc.name, "parenttype": "Stock Valuation Adjustment"},
...
def _queue_propagation_entry(
	*,
	...
	doc = frappe.get_doc(
		{
			"doctype": "Stock Valuation Propagation Entry",
			"adjustment": adjustment,
	...
	try:
		...
		existing = frappe.db.sql(
			"""
			SELECT name
			FROM `tabStock Valuation Propagation Entry`
			WHERE idempotency_key=%s
			LIMIT 1
			FOR UPDATE
			""",
...
def _apply_propagation_entry(entry_name):
	...
	routing = frappe.db.get_value(
		"Stock Valuation Propagation Entry", entry_name, ["target_sle"], as_dict=True
	)
	if not routing:
		return
	...
	targets = frappe.db.sql(
		"SELECT * FROM `tabStock Ledger Entry` WHERE name=%s FOR UPDATE",
		(routing.target_sle,),
		as_dict=True,
	)
	if not targets:
...
def _get_sle(name):
	...
	return frappe._dict(frappe.db.get_value("Stock Ledger Entry", name, fields, as_dict=True))


def _update_receipt_current_value(target):
	"""Expose the effective receipt value without overwriting submitted rate."""
	if target.voucher_type != "Goods Received Note" or not target.voucher_detail_no:
		return
	meta = frappe.get_meta("Goods Received Note Item")
	if not (
...
def _propagate_outgoing_difference(entry, outgoing_sle, difference):
	...
	paired = frappe.db.get_value(
		"Stock Ledger Entry", outgoing_sle, "paired_stock_ledger_entry"
	)
	if paired:
		paired_row = frappe.db.get_value(
			"Stock Ledger Entry", paired, ["qty", "is_cancelled"], as_dict=True
		)
...
def _requires_persisted_lineage(outgoing_sle):
	...
	if not row:
		return False
	if row.voucher_type in {"Delivery Challan", "Goods Received Note", "Work Order"}:
		return True
...
def _refresh_progress(adjustment, status=None):
	total = frappe.db.count("Stock Valuation Propagation Entry", {"adjustment": adjustment})
...
def _complete_adjustment(adjustment):
	reversal_of = frappe.db.get_value(
		"Stock Valuation Adjustment", adjustment, "reversal_of"
	)
...
def _clear_stale_flags(adjustments, targets):
	...
	for target_sle in targets:
		other_active = frappe.db.sql(
			...
			FROM `tabStock Valuation Propagation Entry` propagation
			INNER JOIN `tabStock Valuation Adjustment` adjustment
...
def _set_adjustment_values(name, **values):
	frappe.db.set_value(
		"Stock Valuation Adjustment", name, values, update_modified=False
	)


def _mark_failed(adjustment, phase, error_log):
	if not frappe.db.exists("Stock Valuation Adjustment", adjustment):
		return
	status = "Calculation Failed" if phase == "Calculation" else "Apply Failed"
	retry_count = cint(
		frappe.db.get_value("Stock Valuation Adjustment", adjustment, "retry_count")
	) + 1
	_set_adjustment_values(
		adjustment,
		status=status,
...
def retry_adjustment(adjustment):
	doc = frappe.get_doc("Stock Valuation Adjustment", adjustment)
...
def register_production_links(source_doctype, source_name, links):
	...
	for row in links or []:
		...
		existing = frappe.db.get_value(
			"Stock Valuation Production Link", {"idempotency_key": key}, "name"
		)
		if existing:
			frappe.db.set_value(
				"Stock Valuation Production Link",
