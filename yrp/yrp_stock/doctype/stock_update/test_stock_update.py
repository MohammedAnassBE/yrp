...
from yrp.yrp_stock.doctype.stock_update.stock_update import get_stock_update_rates
...
class TestStockUpdateRateValidation(FrappeTestCase):
	...
	def test_editor_rate_lookup_is_bucket_aware(self):
		with (
			patch(
				"yrp.yrp_stock.doctype.stock_update.stock_update.frappe.has_permission"
			) as has_permission,
			patch(
		...
		has_permission.assert_any_call("Stock Update", "create", throw=True)
