...
class TestStockValuationClosing(IntegrationTestCase):
	def _closing(self, closing_date="2026-08-20"):
		return frappe.get_doc(
			{
				"doctype": "Stock Valuation Closing",
	...
	def test_schema_is_submittable_and_settings_cutoff_is_read_only(self):
		closing_meta = frappe.get_meta("Stock Valuation Closing")
	...
	@patch(
		"yrp.yrp_stock.doctype.stock_valuation_closing.stock_valuation_closing.get_closing_snapshot"
	)
	def test_submit_and_cancel_lifecycle_updates_real_settings_cutoff(self, snapshot):
		if frappe.db.exists("Stock Valuation Closing", {"docstatus": 1}):
	...
	@patch(
		"yrp.yrp_stock.doctype.stock_valuation_closing.stock_valuation_closing.get_latest_submitted_closing"
	)
	...
	@patch(
		"yrp.yrp_stock.doctype.stock_valuation_closing.stock_valuation_closing._set_settings_cutoff"
	)
	...
	@patch(
		"yrp.yrp_stock.doctype.stock_valuation_closing.stock_valuation_closing._lock_closing_state"
	)
	...
	@patch(
		"yrp.yrp_stock.doctype.stock_valuation_closing.stock_valuation_closing._get_latest_valuation_rows",
		return_value=[],
	)
	@patch(
		"yrp.yrp_stock.doctype.stock_valuation_closing.stock_valuation_closing._get_negative_stock_bucket_count",
		return_value=0,
	)
...
class TestStockValuationPeriodGuard(IntegrationTestCase):
	...
	def test_blank_cutoff_skips_validation(self, _closing_date):
		validate_stock_valuation_period(
			"2026-01-01", "Stock Entry", "STE-TEST"
		)
	...
	def test_date_on_cutoff_is_blocked(self, _closing_date):
		with self.assertRaises(StockValuationPeriodClosedError):
			validate_stock_valuation_period(
				"2026-07-31", "Stock Entry", "STE-CLOSED"
			)
	...
	def test_date_after_cutoff_is_allowed(self, _closing_date):
		validate_stock_valuation_period(
			"2026-08-01", "Stock Entry", "STE-OPEN"
		)
	...
	def test_doctype_json_grants_cancel_to_stock_manager(self):
		path = frappe.get_app_path(
			...
			"yrp_stock",
			"doctype",
			"stock_valuation_closing",
			"stock_valuation_closing.json",
		)
