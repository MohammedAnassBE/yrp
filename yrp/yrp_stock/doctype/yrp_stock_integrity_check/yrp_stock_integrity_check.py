...
class YRPStockIntegrityCheck(Document):
	def on_submit(self):
...
def _detect_missing_bins(dim_fields):
	...
	rows = frappe.db.sql(
		...
		SELECT sle.item AS item_code, sle.warehouse,
		       ...
		       {dim_cols}
		FROM `tabYRP Stock Ledger Entry` sle
		LEFT JOIN `tabYRP Bin` b
...
def fix_integrity_row(parent_name, row_name):
	...
	live_sum = (
		frappe.db.sql(
			f"SELECT COALESCE(SUM(qty), 0) FROM `tabYRP Stock Ledger Entry` WHERE {' AND '.join(conds)}",
			tuple(values),
		)[0][0]
		or 0
	)
...
def fix_all_integrity(parent_name):
	...
	frappe.enqueue(
		"yrp.yrp_stock.doctype.yrp_stock_integrity_check.yrp_stock_integrity_check._fix_all_worker",
...
StockIntegrityCheck = YRPStockIntegrityCheck
