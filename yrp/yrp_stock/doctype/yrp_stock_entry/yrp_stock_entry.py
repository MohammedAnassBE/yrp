...
class YRPStockEntry(Document):
	def onload(self):
		from yrp.stock.save_stock_items import group_items_for_ui

		grouped = group_items_for_ui(self.get("items") or [], 'YRP Stock Entry')
	...
	def before_validate(self):
		...
		if self.get("item_details") and self._action != "submit":
			rows = ungroup_items_from_ui(self.item_details, 'YRP Stock Entry')
			self.set("items", [])
			for r in rows:
				self.append("items", r)
	...
	def set_receive_links(self):
		...
		source_doc = frappe.get_doc('YRP Stock Entry', self.outgoing_stock_entry)
		dim_fields = get_dimension_fieldnames()
	...
	def validate_grn_completion_pending(self):
		if self.purpose != "GRN Completion" or self.against != 'YRP Goods Received Note' or not self.against_id:
			return
		for row in self.items:
	...
	def validate_dc_completion_pending(self):
		if self.purpose != "DC Completion" or self.against != 'YRP Delivery Challan' or not self.against_id:
			return
		for row in self.items:
			if not row.against_id_detail:
	...
	def update_transferred_qty(self):
		...
		for source_row in source_doc.items:
			transferred = frappe.db.sql(
				...
				FROM `tabYRP Stock Entry Detail`
				WHERE against_stock_entry = %s
				AND ste_detail = %s
				AND docstatus = 1""",
				(self.outgoing_stock_entry, source_row.name),
			)[0][0]
			frappe.db.set_value('YRP Stock Entry Detail', source_row.name, "transferred_qty", flt(transferred), update_modified=False)
		...
		frappe.db.set_value('YRP Stock Entry', self.outgoing_stock_entry, "per_transferred", flt(per_transferred), update_modified=False)
	...
	def update_dc_completion(self, cancel=False):
		...
		dc = frappe.get_doc('YRP Delivery Challan', dc_name, for_update=True)
	...
	def update_grn_completion(self, cancel=False):
		...
		grn = frappe.get_doc('YRP Goods Received Note', grn_name, for_update=True)
...
def make_stock_in_entry(source_name, target_doc=None):
	...
	doclist = get_mapped_doc(
		...
		source_name,
		{
			...
			'YRP Stock Entry Detail': {
				"doctype": 'YRP Stock Entry Detail',
				"field_map": {
					"name": "ste_detail",
	...
	grouped = group_items_for_ui(doclist.get("items") or [], 'YRP Stock Entry')
	doclist.set_onload("item_details", grouped)

	return doclist


StockEntry = YRPStockEntry
