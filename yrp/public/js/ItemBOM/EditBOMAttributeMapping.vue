    ...
    methods: {
        ...
        set_attributes:async function(attributes) {
            ...
            if (attributes && attributes.length > 0) {
                // Build data from item attribute combinations locally
                let item_attr_values = {};
                for (let i = 0; i < attributes.length; i++) {
                    if (attributes[i].type == 'item' && attributes[i].attribute_values) {
                        item_attr_values[attributes[i].attribute] = attributes[i].attribute_values;
                    }
                }
                // Generate combinations from item attributes
                let combinations = [{}];
                for (let attr of me.item_attributes) {
                    let values = item_attr_values[attr] || [];
                    let new_combinations = [];
                    for (let combo of combinations) {
                        for (let val of values) {
                            let new_combo = Object.assign({}, combo);
                            new_combo[me.get_attribute_name('item', attr)] = val;
                            new_combo['included'] = true;
                            new_combinations.push(new_combo);
                        }
                    }
                    combinations = new_combinations;
                }
                // Add bom attribute placeholders
                for (let combo of combinations) {
                    for (let attr of me.bom_attributes) {
                        combo[me.get_attribute_name('bom', attr)] = null;
                    }
                    combo['quantity'] = 0;
                }
                me.data = combinations;
                me.$nextTick(() => {
