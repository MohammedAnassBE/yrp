...
class YRPYRPWhatsAppHubSettings(Document):
	...
	def _sync_local_whatsapp_accounts(self):
		for row in (self.get("accounts") or []):
			...
			if frappe.db.exists('YRP YRP WhatsApp Account', name):
				frappe.db.set_value('YRP YRP WhatsApp Account', name, {
...
YRPWhatsAppHubSettings = YRPYRPWhatsAppHubSettings
