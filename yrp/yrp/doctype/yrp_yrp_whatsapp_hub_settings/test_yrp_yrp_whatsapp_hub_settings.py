...
class TestYRPHubSettingsAccountAutosync(IntegrationTestCase):
	...
	def tearDown(self):
		if frappe.db.exists('YRP YRP WhatsApp Account', self.TEST_ACCT):
			frappe.delete_doc('YRP YRP WhatsApp Account', self.TEST_ACCT,
	...
	def test_on_update_creates_missing_whatsapp_account(self):
		...
		self.assertTrue(
			frappe.db.exists('YRP YRP WhatsApp Account', self.TEST_ACCT),
			"YRP WhatsApp Account should be auto-created from Hub Settings child row",
		)
		acct = frappe.get_doc('YRP YRP WhatsApp Account', self.TEST_ACCT)
		self.assertEqual(int(acct.is_default), 1)

	def test_on_update_updates_existing_whatsapp_account(self):
		if not frappe.db.exists('YRP YRP WhatsApp Account', self.TEST_ACCT):
			frappe.get_doc({
		...
		self.assertEqual(
			int(frappe.db.get_value('YRP YRP WhatsApp Account', self.TEST_ACCT, "is_default")),
			1,
		)
