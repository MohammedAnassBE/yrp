...
SEND_REQUEST = "yrp.yrp.doctype.yrp_notification_template.yrp_notification_template.send_request"
...
class _NotificationTestBase(FrappeTestCase):
	...
	def setUp(self):
		super().setUp()
		frappe.db.delete('YRP Notification Template')
...
def _notification_template(name, channel="SMS", document_type='YRP Purchase Order',
	...
	template = frappe.get_doc({
		"doctype": 'YRP Notification Template',
		"__newname": name,
		"enabled": 1,
		"channel": channel,
...
class TestNotificationAPI(_NotificationTestBase):
	...
	def test_send_sms_notification_rejects_foreign_template(self):
		...
		wrong = _notification_template(
			f"_T DC SMS {frappe.generate_hash(length=6)}", document_type='YRP Delivery Challan'
		)
		with patch(SEND_REQUEST, return_value=_gw(200)) as mock_send:
			with self.assertRaises(frappe.ValidationError):
