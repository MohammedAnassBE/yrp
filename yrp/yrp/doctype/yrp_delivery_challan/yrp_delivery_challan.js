...
function add_complete_transfer_button(frm) {
	...
	frm.add_custom_button(__("Complete Transfer"), () => {
		frappe.call({
			method: "yrp.yrp.doctype.yrp_delivery_challan.yrp_delivery_challan.make_dc_completion",
...
function add_return_button(frm) {
	if (frm.doc.docstatus !== 1 || !frappe.model.can_create("YRP Goods Received Note")) {
		return;
	}
...
function show_return_dialog(frm, data) {
	...
	const dialog = new frappe.ui.Dialog({
		...
		primary_action(values) {
			...
			frappe.call({
				method: "yrp.yrp.doctype.yrp_delivery_challan.yrp_delivery_challan.create_return_grn",
				args: {
