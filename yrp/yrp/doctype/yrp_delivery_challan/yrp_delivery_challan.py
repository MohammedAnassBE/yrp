...
class YRPDeliveryChallan(Document):
	def onload(self):
...
def _lock_work_order_deliverables(work_order):
	...
	frappe.db.sql(
		"""
			SELECT name
			FROM `tabYRP Work Order Deliverables`
			WHERE parent = %s
			ORDER BY name
			FOR UPDATE
		""",
	...
	delivered = {
		...
		for row in frappe.db.sql(
			"""
				...
				FROM `tabYRP Delivery Challan Item` item
				INNER JOIN `tabYRP Delivery Challan` dc ON dc.name = item.parent
				WHERE dc.work_order = %(work_order)s
				  AND dc.docstatus = 1
				  AND item.ref_doctype = 'YRP Work Order Deliverables'
	...
	returned = {
		...
		for row in frappe.db.sql(
			"""
				...
				INNER JOIN `tabYRP Goods Received Note` grn ON grn.name = item.parent
...
DeliveryChallan = YRPDeliveryChallan
