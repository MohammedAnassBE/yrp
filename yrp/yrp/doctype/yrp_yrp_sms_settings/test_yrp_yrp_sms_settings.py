...
class TestYRPSMSSettings(FrappeTestCase):
	...
	def _configure(self, rows, gateway_url="https://control.msg91.com/api/v5/flow/",
		...
		doc = frappe.get_doc('YRP YRP SMS Settings')
		doc.enabled = enabled
		...
		frappe.clear_cache(doctype='YRP YRP SMS Settings')
	...
	def test_deliver_no_config_captured_as_failure(self):
		...
		with patch(POST) as p:
			result = deliver_flow_sms(reference_doctype='YRP Stock Entry', mobile_no="9944405056")
		self.assertFalse(result["ok"])
