...
frappe.ui.form.on("YRP Purchase Invoice", {
	...
	refresh(frm) {
		...
		if (frm.doc.docstatus === 0 && frm.doc.against === "YRP Work Order" && !frm.doc.approved_by) {
...
function approve_invoice(frm) {
	const d = new frappe.ui.Dialog({
		...
		primary_action(values) {
			d.hide();
			frappe.call({
				method: "yrp.yrp.doctype.yrp_purchase_invoice.yrp_purchase_invoice.approve_invoice",
...
function render_work_order_details(frm) {
	...
	frappe.call({
		method: "yrp.yrp.doctype.yrp_purchase_invoice.yrp_purchase_invoice.get_merch_roles",
		callback(role_res) {
			frappe.call({
				method: "yrp.yrp.doctype.yrp_work_order.yrp_work_order.get_close_permission",
				callback(permission_res) {
					frappe.call({
						method: "yrp.yrp.doctype.yrp_purchase_invoice.yrp_purchase_invoice.check_all_wo_closed",
						...
						callback(status_res) {
							...
							if (frm.doc.docstatus === 0 && !frm.doc.approved_by && frm.doc.against === "YRP Work Order") {
...
function render_debit_summary(frm) {
	...
	for (const d of debits) {
		...
		html += `<tr>
			...
			<td><a href="/app/yrp-debit/${encodeURIComponent(d.name)}" target="_blank">${frappe.utils.escape_html(d.name)}</a></td>
	...
	const d = new frappe.ui.Dialog({
		...
		primary_action(values) {
			...
			const close_work_order = () => {
				frappe.call({
					method: "yrp.yrp.doctype.yrp_work_order.yrp_work_order.update_stock",
					args: {
						work_order,
			...
			if (values.with_debit === "With Debit") {
				frappe.call({
					method: "yrp.yrp.doctype.yrp_debit.yrp_debit.create_debit",
					args: {
						work_order,
...
function render_close_debits(work_order, dialog) {
	frappe.call({
		method: "yrp.yrp.doctype.yrp_work_order.yrp_work_order.get_debits",
		args: { work_order },
		callback(r) {
