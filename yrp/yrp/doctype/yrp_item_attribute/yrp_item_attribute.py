...
class YRPItemAttribute(Document):
	...
	def create_numeric_values(self):
		...
		if from_range and to_range and increment:
			...
			while cur_val <= to_range:
				if not frappe.db.exists('YRP Item Attribute Value', remove_suffix(str(cur_val), '.0') + ' ' +  self.name):
					frappe.get_doc({
	...
	def onload(self):
		...
		attribute_values = frappe.get_all('YRP Item Attribute Value', filters=filters, fields=["*"])
...
ItemAttribute = YRPItemAttribute
