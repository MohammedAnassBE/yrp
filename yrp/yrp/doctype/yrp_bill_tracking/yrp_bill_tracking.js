frappe.ui.form.on("YRP Bill Tracking", {
	async refresh(frm) {
		...
		if (frm.doc.docstatus === 0) {
			setup_delivery_person_suggestion(frm);
			if (!frm.is_new()) {
		...
		if (!frm.is_new() && frm.doc.docstatus === 1) {
			...
			if (can_receive && frm.doc.form_status !== "Closed") {
				frm.add_custom_button(__("Bill Received"), () => {
					frappe.call({
						method: "yrp.yrp.doctype.yrp_bill_tracking.yrp_bill_tracking.make_bill_received_acknowledgement",
	...
	supplier(frm) {
		...
		if (frm.delivery_person_suggestion) {
			frm.delivery_person_suggestion.update_for_new_supplier(frm.doc.supplier);
		}
	},
});

function setup_delivery_person_suggestion(frm) {
	const wrapper = frm.fields_dict["delivery_person_suggestion_html"]?.wrapper;
	const Widget = frappe.yrp?.ui?.SuggestedBillDeliveryPerson;
	if (!wrapper || !Widget) return;
	frm.delivery_person_suggestion = new Widget(wrapper);
}
...
async function can_show_bill_received(frm) {
	...
	let last_item = null;
	for (const row of frm.doc.bill_tracking_history || []) {
		if (row.assigned_to === frm.doc.assigned_to) last_item = row;
	}
	if (!last_item || last_item.received) return false;
	return await new Promise((resolve) => {
		frappe.call({
			method: "yrp.yrp.doctype.yrp_bill_tracking.yrp_bill_tracking.check_for_can_show_receive_btn",
...
function show_assign_dialog(frm) {
	const dialog = new frappe.ui.Dialog({
		...
		primary_action: (values) => {
			frappe.call({
				method: "yrp.yrp.doctype.yrp_bill_tracking.yrp_bill_tracking.assign_vendor_bill",
				args: { name: frm.doc.name, assigned_to: values.assigned_to, remarks: values.remarks },
				freeze: true,
				freeze_msg: __("Assigning Document"),
				callback: () => {
					dialog.hide();
					frm.reload_doc();
				},
...
function show_cancel_dialog(frm) {
	const dialog = new frappe.ui.Dialog({
		...
		primary_action: (values) => {
			frappe.call({
				method: "yrp.yrp.doctype.yrp_bill_tracking.yrp_bill_tracking.cancel_vendor_bill",
				args: { name: frm.doc.name, cancel_reason: values.cancel_reason },
				freeze: true,
				freeze_msg: __("Cancelling Document"),
				callback: () => {
					dialog.hide();
					frm.reload_doc();
				},
