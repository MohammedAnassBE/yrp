frappe.listview_settings["YRP Bill Tracking"] = {
	onload: (listview) => {
		listview.page.add_action_item(__("Bulk Assign"), () => {
			const dialog = new frappe.ui.Dialog({
				...
				primary_action: (values) => {
					dialog.hide();
					frappe.call({
						method: "yrp.yrp.doctype.yrp_bill_tracking.yrp_bill_tracking.bulk_assign_bills",
						args: {
	...
	refresh: (listview) => {
		listview.page.add_inner_button(__("Show Assigned To Me"), async () => {
			...
			frappe.call({
				method: "yrp.yrp.doctype.yrp_department.yrp_department.get_user_departments",
				callback: async (r) => {
					await listview.filter_area.set([
						["YRP Bill Tracking", "assigned_to", "in", r.message || []],
						["YRP Bill Tracking", "docstatus", "=", 1],
						["YRP Bill Tracking", "form_status", "!=", "Closed"],
					]);
					listview.refresh();
				},
			});
