...
frappe.ui.form.on("YRP Process Cost", {
	setup(frm) {
		frm.set_query("attribute", function () {
			return {
				query: "yrp.yrp.doctype.yrp_process_cost.yrp_process_cost.get_item_attributes",
				filters: {
					item: frm.doc.item,
				},
			};
	...
	attribute(frm) {
		if (frm.doc.attribute && frm.doc.item) {
			frappe.call({
				method: "yrp.yrp.doctype.yrp_process_cost.yrp_process_cost.get_pc_attribute_values",
				args: {
					item: frm.doc.item,
