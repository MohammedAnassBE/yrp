...
class TestDeliveryChallanPendingRebuild(UnitTestCase):
	def test_rebuild_uses_submitted_deliveries_less_returns(self):
		...
		def sql(query, *_args, **_kwargs):
			if "FROM `tabDelivery Challan Item`" in query:
				return [frappe._dict(ref_docname="DEL-1", quantity=8)]
			if "FROM `tabGoods Received Note Item`" in query:
				return [frappe._dict(ref_docname="DEL-1", quantity=2)]
			return []

		with (
			patch("yrp.yrp.doctype.delivery_challan.delivery_challan.frappe.db.sql", side_effect=sql),
			patch("yrp.yrp.doctype.delivery_challan.delivery_challan.frappe.get_doc", return_value=work_order),
			patch("yrp.yrp.doctype.delivery_challan.delivery_challan._update_work_order_status"),
		):
