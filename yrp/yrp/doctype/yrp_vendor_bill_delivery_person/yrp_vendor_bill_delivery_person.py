...
class YRPVendorBillDeliveryPerson(Document):
	def validate(self):
...
def get_last_ten_delivery_persons(supplier=None, search=""):
	...
	return frappe.db.sql(
		...
		FROM `tabYRP Vendor Bill Delivery Person`
		WHERE mobile_no LIKE %(search)s or person_name LIKE %(search)s
		LIMIT 10
	""",
		{"supplier": supplier, "search": f"%{search}%"},
		as_dict=True,
	)


VendorBillDeliveryPerson = YRPVendorBillDeliveryPerson
