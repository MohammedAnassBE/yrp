...
frappe.ui.form.on("YRP IPD Process Matrix", {
	setup(frm) {
		frm.set_query("reference_item_variant", (doc) => {
			...
			return {
				query: "yrp.yrp.doctype.yrp_ipd_process_matrix.yrp_ipd_process_matrix.get_reference_variant_query",
				filters: {
					ipd: doc.ipd,
				},
			};
...
function mount_editor(frm) {
	...
	if (frm.doc.ipd) {
		frappe.db.get_value("YRP Item Production Detail", frm.doc.ipd, "dependent_attribute").then((r) => {
