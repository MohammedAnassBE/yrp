frappe.ui.form.on("YRP Work Order Correction", {
	setup(frm) {
...
function mount_correction_editor(frm, config) {
	if (!frappe.yrp.work_order || !frappe.yrp.work_order.ItemEditor || !frm.fields_dict[config.fieldname]) {
		return;
	}
