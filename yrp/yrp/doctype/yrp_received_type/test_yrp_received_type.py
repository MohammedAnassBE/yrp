...
class TestReceivedType(FrappeTestCase):
	def test_only_one_default(self):
		a = frappe.get_doc(
			{"doctype": 'YRP Received Type', "received_type_name": "_Test RT A", "is_default": 1}
		).insert()
		b = frappe.get_doc(
			{"doctype": 'YRP Received Type', "received_type_name": "_Test RT B", "is_default": 1}
		).insert()
		self.assertEqual(frappe.db.get_value('YRP Received Type', a.name, "is_default"), 0)
		self.assertEqual(frappe.db.get_value('YRP Received Type', b.name, "is_default"), 1)
