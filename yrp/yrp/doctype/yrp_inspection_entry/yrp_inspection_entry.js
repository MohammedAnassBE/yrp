...
frappe.ui.form.on("YRP Inspection Entry", {
	setup(frm) {
		frm.set_query("against_id", () => {
			const base = { docstatus: 1 };
			if (frm.doc.against === "YRP Goods Received Note") {
				base.is_rework = 0;
			}
			if (frm.doc.against === "YRP Stock Entry") {
	...
	against_id(frm) {
		...
		frappe.call({
			method: "yrp.yrp.doctype.yrp_inspection_entry.yrp_inspection_entry.get_initial_payload",
...
function _maybe_add_convert_stock_button(frm) {
	...
	frappe.call({
		method: "yrp.yrp.doctype.yrp_inspection_entry.yrp_inspection_entry.can_convert_stock",
...
function _open_convert_stock_dialog(frm, siblings) {
	...
	if (siblings.length) {
		...
		for (const s of siblings) {
			const link = `<a href="/app/yrp-inspection-entry/${encodeURIComponent(s.name)}" target="_blank">${escape(s.name)}</a>`;
	...
	const d = new frappe.ui.Dialog({
		...
		primary_action() {
			d.hide();
			frappe.call({
				method: "yrp.yrp.doctype.yrp_inspection_entry.yrp_inspection_entry.convert_stock",
				args: { name: frm.doc.name },
				freeze: true,
