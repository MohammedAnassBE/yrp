...
from yrp.yrp.doctype.excel_sticker_print.excel_sticker_print import (
	get_print_format,
	get_raw_code,
)
...
class TestExcelStickerPrint(IntegrationTestCase):
	def test_preview_reads_the_yrp_zpl_print_format(self):
		...
		self.assertEqual(
			...
			[
				call("Excel Sticker Print", "ESP-0001"),
				call("ZPL Raw Print Format", "ZPL-Test"),
			],
		)
	...
	def test_client_has_no_production_api_dependency(self):
		client_path = frappe.get_app_path(
			"yrp", "yrp", "doctype", "excel_sticker_print", "excel_sticker_print.js"
		)
		...
		self.assertIn(
			"yrp.yrp.doctype.excel_sticker_print.excel_sticker_print.get_raw_code",
			client,
		)
		self.assertIn(
			"yrp.yrp.doctype.excel_sticker_print.excel_sticker_print.get_print_format",
			client,
		)
