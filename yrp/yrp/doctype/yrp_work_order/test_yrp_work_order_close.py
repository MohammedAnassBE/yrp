...
class TestWorkOrderClose(FrappeTestCase):
	...
	def test_unapproved_debit_blocks_close_until_approved(self):
		...
		debit = frappe.get_attr("yrp.yrp.doctype.yrp_debit.yrp_debit.create_debit")(
			wo.name,
		...
		frappe.get_attr("yrp.yrp.doctype.yrp_debit.yrp_debit.approve_debit")(debit.name)
		status = frappe.get_attr("yrp.yrp.doctype.yrp_work_order.yrp_work_order.update_stock")(
			wo.name,
