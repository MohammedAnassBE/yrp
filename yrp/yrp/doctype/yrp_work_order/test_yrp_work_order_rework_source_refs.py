...
class TestWorkOrderReworkSourceRefs(FrappeTestCase):
	def test_rework_deliverable_requires_grn_source_ref(self):
		doc = _rework_work_order()
		with self.assertRaisesRegex(frappe.ValidationError, "must reference a Source GRN Item"):
			doc.validate_rework_source_refs()

	def test_rework_deliverable_accepts_grn_source_ref(self):
