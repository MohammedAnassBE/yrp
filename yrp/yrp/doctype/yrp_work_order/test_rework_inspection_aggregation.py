...
class TestReworkInspectionAggregation(FrappeTestCase):
	...
	def test_multiple_inspections_aggregate_by_grn_item_and_bucket(self):
		...
		with (
			...
			patch(
				"yrp.yrp.doctype.yrp_work_order.yrp_work_order._eligible_rt_context",
				return_value=("Accepted", "Rejected"),
			),
			patch(
				"yrp.yrp.doctype.yrp_work_order.yrp_work_order._prior_rework_consumed",
				return_value=0,
			),
			patch(
				"yrp.yrp.doctype.yrp_work_order.yrp_work_order._item_uom",
				return_value="Kg",
			),
		):
