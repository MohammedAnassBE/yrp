...
def _inspection_rework_sources(grn_by_name):
	...
	conditions = [
		"p.against = 'YRP Goods Received Note'",
		f"p.against_id IN ({placeholders})",
		"p.docstatus = 1",
		"(p.is_converted = 1 OR p.status = 'Converted')",
		"i.parenttype = 'YRP Inspection Entry'",
		"i.ref_doctype = 'YRP Goods Received Note Item'",
		"g.parenttype = 'YRP Goods Received Note'",
	...
	rows = frappe.db.sql(
		...
		SELECT g.name AS source_grn_item,
		       ...
		       {select_sql}
		FROM `tabYRP Inspection Entry Item` i
		JOIN `tabYRP Inspection Entry` p ON p.name = i.parent
		JOIN `tabYRP Goods Received Note Item` g ON g.name = i.ref_docname
...
def _inspection_outflow_from_grn_row(grn_item_name):
	...
	row = frappe.db.sql(
		...
		FROM `tabYRP Inspection Entry Item` i
		JOIN `tabYRP Inspection Entry` p ON p.name = i.parent
		WHERE i.ref_doctype = 'YRP Goods Received Note Item'
		  AND i.ref_docname = %s
		  AND p.docstatus = 1
