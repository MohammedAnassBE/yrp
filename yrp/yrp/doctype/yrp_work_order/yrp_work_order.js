...
function add_create_debit_button(frm) {
	...
	if (!frappe.model.can_create("YRP Debit")) return;
	frm.add_custom_button(__("Create Debit"), () => {
		frappe.new_doc("YRP Debit", { work_order: frm.doc.name });
	});
}
...
function add_create_rework_button(frm) {
	...
	frm.add_custom_button(__("Create Rework"), () => {
		frappe.call({
			method: "yrp.yrp.doctype.yrp_work_order.yrp_work_order.get_rework_source_rows",
...
function open_create_rework_dialog(frm, sources) {
	...
	const d = new frappe.ui.Dialog({
		...
		primary_action(values) {
			...
			frappe.call({
				method: "yrp.yrp.doctype.yrp_work_order.yrp_work_order.create_rework_work_order",
				args: {
	...
	frappe.call({
		...
		callback(r) {
			...
			for (const debit of debits) {
				html += `<tr>
					<td><a href="/app/yrp-debit/${encodeURIComponent(debit.name)}" target="_blank">${frappe.utils.escape_html(debit.name)}</a></td>
