...
function set_primary_address(frm, supplier, targetField) {
	...
	frappe.call({
		method: "yrp.yrp.doctype.yrp_supplier.yrp_supplier.get_primary_address",
		args: { supplier },
		callback(r) {
...
function show_create_dc_dialog(frm) {
	const d = new frappe.ui.Dialog({
		...
		primary_action(values) {
			...
			frappe.set_route("Form", "YRP Delivery Challan", "new").then(() => {
				...
				if (cur_frm && cur_frm.doctype === "YRP Delivery Challan") {
