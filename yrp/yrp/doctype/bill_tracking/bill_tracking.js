frappe.ui.form.on("Bill Tracking", {
	async refresh(frm) {
		if (frm.is_new()) {
		...
		if (!frm.is_new() && frm.doc.docstatus === 1) {
			...
			if (can_receive && frm.doc.form_status !== "Closed") {
				frm.add_custom_button(__("Bill Received"), () => {
					frappe.call({
						method: "yrp.yrp.doctype.bill_tracking.bill_tracking.make_bill_received_acknowledgement",
...
async function can_show_bill_received(frm) {
	...
	return await new Promise((resolve) => {
		frappe.call({
			method: "yrp.yrp.doctype.bill_tracking.bill_tracking.check_for_can_show_receive_btn",
...
function show_assign_dialog(frm) {
	const dialog = new frappe.ui.Dialog({
		...
		primary_action: (values) => {
			frappe.call({
				method: "yrp.yrp.doctype.bill_tracking.bill_tracking.assign_vendor_bill",
...
function show_cancel_dialog(frm) {
	const dialog = new frappe.ui.Dialog({
		...
		primary_action: (values) => {
			frappe.call({
				method: "yrp.yrp.doctype.bill_tracking.bill_tracking.cancel_vendor_bill",
