...
def get_data(filters):
	...
	query = f"""
		...
		FROM `tabYRP Bill Tracking` t1
		LEFT JOIN (
			SELECT t2.*
			FROM `tabYRP Bill Tracking Assignment Detail` t2
			INNER JOIN (
				SELECT inner_child.parent, MAX(inner_child.idx) AS max_idx
				FROM `tabYRP Bill Tracking Assignment Detail` inner_child
				JOIN `tabYRP Bill Tracking` inner_parent ON inner_parent.assigned_to = inner_child.assigned_to
				GROUP BY parent
	...
	return frappe.db.sql(query, {"curr_date": today()}, as_dict=True)
