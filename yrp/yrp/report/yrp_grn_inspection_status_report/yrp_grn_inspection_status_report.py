...
def _get_inspection_map(grn_items):
	...
	rows = frappe.db.sql(
		...
		FROM `tabYRP Inspection Entry Item` iei
		INNER JOIN `tabYRP Inspection Entry` ie ON ie.name = iei.parent
		WHERE ie.docstatus = 1
		  AND ie.against = 'YRP Goods Received Note'
		  AND iei.ref_doctype = 'YRP Goods Received Note Item'
